...
__forceinline uint32 crc32_update(uint32 crc, uint8 byte)
{
...
__forceinline void prep_huffman(vorb *f)
{
   if (f->valid_bits <= 24) {
      if (f->valid_bits == 0) f->acc = 0;
      do {
...
__forceinline void draw_line(float *output, int x0, int y0, int x1, int y1, int n)
{
   int dy = y1 - y0;
   int adx = x1 - x0;
   int ady = abs(dy);
...
__forceinline void iter_54(float *z)
{
