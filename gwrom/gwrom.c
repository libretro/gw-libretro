...
/* use gwrom allocation routines */
void zfree( void* opaque, void* addr ) { gwrom_free( addr ); }
...
static int decompress_gzip( void** new_data,
      size_t* new_size, void* data, size_t size )
{
   int res;
   uLong dest_len;
   ...
   *new_data = gwrom_malloc( *new_size );
   ...
   /* decompress */
   dest_len = *new_size;
   res      = uncompress( *new_data, &dest_len, data, size );
...
/* use gwrom allocation routines */
void* bzalloc( void* opaque, Int32 items, Int32 size ) { return gwrom_malloc( items * size ); }
/* use gwrom allocation routines */
void bzfree( void* opaque, void* addr ) { gwrom_free( addr ); }
...
static int decompress_bzip2( void** new_data, size_t* new_size,
      void* data, size_t size )
{
   ...
   *new_size = 0;
   ...
   for ( ;; )
   {
      ...
      count = sizeof( buffer ) - stream.avail_out;
...
static int inflate_uncompressed( void** new_data,
      size_t* new_size, void* data, size_t size )
{
  /*
...
#if 0
static int default_init( gwrom_t* gwrom ) { return GWROM_OK; }
static int default_find( gwrom_entry_t* file, gwrom_t* gwrom,
      const char* file_name ) { return GWROM_ENTRY_NOT_FOUND; }
#endif
...
static int identify_tar_v7( const void* data, size_t size )
{
   ...
   entry = (entry_tar_v7*)data;
   end   = (char*)data + size - 512;
...
static int find_tar_v7( gwrom_entry_t* file,
      gwrom_t* gwrom, const char* file_name )
{
   entry_tar_v7* entry = (entry_tar_v7*)gwrom->data;
...
static void iterate_tar_v7( gwrom_t* gwrom,
      int (*callback)( gwrom_entry_t*, gwrom_t* ) )
{
   entry_tar_v7* entry = (entry_tar_v7*)gwrom->data;
   gwrom_entry_t file;
...
const char* gwrom_error_message( int error )
{
   switch ( error )
   {
      case GWROM_OK:
         return "Ok";
      case GWROM_INVALID_ROM:
         return "Invalid ROM (corrupted file?)";
      case GWROM_NO_MEMORY:
         return "Out of memory";
      case GWROM_ENTRY_NOT_FOUND:
